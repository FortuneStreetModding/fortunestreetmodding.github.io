...
def process(lang: str):
    ...
    gameBoardCmpresPath = list(Path().glob(f"game/{langDir}game_board{suffix}.cmpres"))[
        0
    ]
    ...
    for d in dirs_to_check:
        deleteFolderContents(d) if os.path.exists(d) else os.makedirs(d)
    ...
    gameBoardCmpresDecomp = (
        f"{gameBoardCmpresPath.parent.as_posix()}/{gameBoardCmpresPath.stem}_DECOMP.bin"
    )
    print(check_output(f"ntcompress -x {gameBoardCmpres}", encoding="utf-8"))
    print(
        check_output(
            f"wszst EXTRACT {gameBoardCmpresDecomp} --overwrite", encoding="utf-8"
        )
    )
    ...
    print(
        check_output(
            f"wszst EXTRACT {gameBoardArc} --overwrite --dest {gameBoardArcExtractDir}",
            encoding="utf-8",
        )
    )
    ...
    for minimapIcon in minimapIcons:
        ...
        print(
            check_output(
                f"wimgt DECODE {minimapIcon.tpl} --overwrite --dest {minimapIcon.orig}",
                encoding="utf-8",
            )
        )
    ...
    for filePath in filePaths:
        ...
        print(
            check_output(
                f"wimgt DECODE {chanceCardTextureFile} --overwrite", encoding="utf-8"
            )
        )
        ...
        if "ES" in lang or "FR" in lang or "ES" in lang:
            if os.path.exists(f"chance_card_edits/ui_chancecard_{number}_es_fr_it.png"):
                chanceCardEditPngFile = (
                    f"chance_card_edits/ui_chancecard_{number}_es_fr_it.png"
                )
        if os.path.exists(chanceCardEditPngFile):
            ...
            print(
                check_output(
                    f"magick MOGRIFY -alpha off {chanceCardPngFile}", encoding="utf-8"
                )
            )
            print(
                check_output(
                    f"wimgt ENCODE {chanceCardPngFile} --transform .RGB565 --overwrite",
                    encoding="utf-8",
                )
            )
            print(
                check_output(
                    f"wszst CREATE {fileExtractDir} --overwrite --dest {fileDecomp}",
                    encoding="utf-8",
                )
            )
            ...
            print(
                check_output(
                    f"ntcompress -lex {fileDecomp} -o {fileOutput}", encoding="utf-8"
                )
            )
            fileBsdiff = chanceCardBsdiffDir + f"{filePath.stem}.cmpres.bsdiff"
            print(
                check_output(
                    f"bsdiff {file} {fileOutput} {fileBsdiff}", encoding="utf-8"
                )
            )
        ...
        print(
            check_output(
                f"magick MOGRIFY -crop 170x170+5+35 -resize 128x128 -alpha off {imageSquare.format(number=number)}",
                encoding="utf-8",
            )
        )
        ...
        print(
            check_output(
                f"magick MOGRIFY +level 0%,50% -crop 170x170+5+35 -resize 128x128 -alpha off {imageBoard.format(number=number)}",
                encoding="utf-8",
            )
        )
        print(
            check_output(
                f"wimgt ENCODE {imageBoard.format(number=number)} --transform .RGB565 --overwrite --dest {gameBoardTexturesDir}/eventsquare_{number}",
                encoding="utf-8",
            )
        )
        ...
        print(
            check_output(
                f"magick MOGRIFY -crop 170x170+5+35 -resize 29x32! -alpha off {imageIcon.format(number=number)}",
                encoding="utf-8",
            )
        )
        # icon images:
        for minimapIcon in minimapIcons:
            ...
            if int(number) in minimapIconsWhite:
                print(
                    check_output(
                        f"magick CONVERT -size {minimapIcon.resize} canvas:white {imageMinimapIconFile}",
                        encoding="utf-8",
                    )
                )
            else:
                print(
                    check_output(
                        f"magick MOGRIFY -crop 12x12+85+20 -resize {minimapIcon.resize} -alpha off +dither -colors 8 -depth 3 -blur 0x8 -modulate 110,130 {imageMinimapIconFile}",
                        encoding="utf-8",
                    )
                )
            ...
            print(
                check_output(
                    f"magick {imageMinimapIconFile} {minimapIcon.mask} -compose Multiply -composite {imageMinimapIconFile}",
                    encoding="utf-8",
                )
            )
    ...
    print(
        check_output(
            f"wszst CREATE {gameBoardExtractDir} --overwrite --dest {gameBoardCmpresDecomp}",
            encoding="utf-8",
        )
    )
    print(
        check_output(
            f"ntcompress -lex {gameBoardCmpresDecomp} -o {gameBoardOutput}",
            encoding="utf-8",
        )
    )
    print(
        check_output(
            f"bsdiff {gameBoardCmpres} {gameBoardOutput} {gameBoardBsdiff}",
            encoding="utf-8",
        )
    )
    ...
    for minimapIcon in minimapIcons:
        ...
        print(
            check_output(
                f'magick MONTAGE {minimapIcon.tile.format(number="*")} -tile 5x -geometry +0+0 -background transparent {minimapIcon.tmp}',
                encoding="utf-8",
            )
        )
        print(
            check_output(
                f"magick CONVERT -page +0+0 {minimapIcon.orig} -page {minimapIcon.offset} {minimapIcon.tmp} -background transparent -layers merge +repage {minimapIcon.png}",
                encoding="utf-8",
            )
        )
    ...
    with open("output/sha1.yaml", "w", encoding="utf-8") as yaml_file:
        ...
        for lang in ["", "DE", "EN", "ES", "FR", "IT", "UK"]:
            ...
            if lang:
                ...
                langDir = ""
            filePaths = list(Path().glob(f"chance_card/{langDir}*.cmpres"))
            gameBoardCmpresPath = list(
                Path().glob(f"game/{langDir}game_board{suffix}.cmpres")
            )[0]
            fileToSha1[gameBoardCmpresPath.as_posix()] = get_digest(
                gameBoardCmpresPath.as_posix()
            )
            for filePath in filePaths:
                file = filePath.as_posix()
                fileToSha1[file] = get_digest(file)
        dump = yaml.dump(
            fileToSha1, default_flow_style=False, allow_unicode=True, encoding=None
        )
